...
const ConnectionStore_js_1 = require("../../services/ConnectionStore.js");
const router = (0, express_1.Router)();
// ================= CONNECTIONS CONFIGURATION =================
// Get active connection configurations
router.get('/api/connections', async function (req, res) {
    try {
        const savedConnections = await ConnectionStore_js_1.connectionStore.listConnections();
...
router.post('/api/connections/add', async function (req, res) {
    ...
    try {
        const existing = await ConnectionStore_js_1.connectionStore.getConnection(conn_name);
        if (existing) {
        ...
        connPool.addConnection({ connName: conn_name, connString: conn_string, connOptions: options }, req.app, async function (err, data) {
            if (err) {
                ...
                try {
                    await ConnectionStore_js_1.connectionStore.saveConnection(conn_name, conn_string);
                    const savedConnections = await ConnectionStore_js_1.connectionStore.listConnections();
                    const connection_list = {};
                    savedConnections.forEach(conn => {
                        connection_list[conn.name] = {
    ...
    try {
        ...
        connPool.addConnection({ connName: conn_name, connString: conn_string, connOptions: current_options }, req.app, async function (err, data) {
            if (err) {
                ...
                try {
                    const existingRecord = await ConnectionStore_js_1.connectionStore.getConnection(curr_config);
                    if (existingRecord && existingRecord.id) {
                        await ConnectionStore_js_1.connectionStore.deleteConnection(existingRecord.id);
                    }
