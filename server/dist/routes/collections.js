...
// ================= COLLECTIONS =================
// Create collection
router.post('/api/:conn/:db/collection/create', async function (req, res) {
    ...
    try {
        const result = await CollectionService_1.collectionService.createCollection(mongo_db, collection_name);
...
// Rename collection
router.post('/api/:conn/:db/:coll/rename', async function (req, res) {
    const connection_list = MongoService_1.mongoService.getConnections();
    if (!connection_list || !connection_list[req.params.conn]) {
        return res.status(400).json({ 'msg': 'Invalid connection' });
    }
    ...
    const mongo_db = connection_list[req.params.conn].client.db(req.params.db);
    try {
        const result = await CollectionService_1.collectionService.renameCollection(mongo_db, req.params.coll, new_collection_name);
        res.status(200).json(result);
    }
    catch (err) {
...
// Delete collection
router.post('/api/:conn/:db/collection/delete', async function (req, res) {
    const connection_list = MongoService_1.mongoService.getConnections();
    if (!connection_list || !connection_list[req.params.conn]) {
        return res.status(400).json({ 'msg': 'Invalid connection' });
    }
    ...
    const mongo_db = connection_list[req.params.conn].client.db(req.params.db);
    try {
        const result = await CollectionService_1.collectionService.deleteCollection(mongo_db, collection_name);
        res.status(200).json(result);
    }
    catch (err) {
...
// Export collection
router.get('/api/:conn/:db/:coll/export', async function (req, res) {
    const connection_list = MongoService_1.mongoService.getConnections();
    if (!connection_list || !connection_list[req.params.conn]) {
    ...
    const mongo_db = connection_list[req.params.conn].client.db(req.params.db);
    try {
        const data = await CollectionService_1.collectionService.exportCollection(mongo_db, req.params.coll, excludedID);
...
router.get('/api/:conn/:db/:coll/schema', async function (req, res) {
    ...
    const mongo_db = connection_list[req.params.conn].client.db(req.params.db);
    try {
        const fields = await CollectionService_1.collectionService.getSchema(mongo_db, req.params.coll);
...
router.get('/api/:conn/:db/:coll/analysis', async function (req, res) {
    ...
    const mongo_db = connection_list[req.params.conn].client.db(req.params.db);
    try {
        const analysis = await CollectionService_1.collectionService.getAnalysis(mongo_db, req.params.coll);
...
// =============== AI-POWERED ANALYSIS ===============
router.post('/api/:conn/:db/:coll/ai-analysis', async function (req, res) {
    const connection_list = MongoService_1.mongoService.getConnections();
    if (!connection_list || !connection_list[req.params.conn]) {
        return res.status(400).json({ msg: 'Invalid connection' });
    }
    ...
    try {
        const analysis = await CollectionService_1.collectionService.performAiAnalysis(mongo_db, req.params.db, collName, customPrompt);
        res.status(200).json(analysis);
    }
    catch (err) {
