...
router.get('/api/:conn/:db/:coll/schema', async function (req, res) {
    ...
    try {
        ...
        const fields = Object.entries(fieldMap).map(([field, info]) => ({ field, type: info.type, example: info.example }));
...
router.get('/api/:conn/:db/:coll/analysis', async function (req, res) {
    ...
    try {
        ...
        // Convert to summary
        const result = Object.entries(analysis).map(([field, data]) => {
...
Generate 4 meaningful analytical charts for this collection.`;
        ...
        let chartSpecs = [];
        try {
            chartSpecs = await GeminiService_1.geminiService.generateJSON(userPrompt, AI_ANALYSIS_PROMPT);
        ...
        for (let i = 0; i < chartSpecs.length && i < 6; i++) {
            ...
            try {
                ...
                const labelKey = aiLabel
                    || stringKeys[0]
                    || keys[0];
                const valueKey = aiValue
                ...
                if (spec.chartType === 'pie') {
                    option = {
                        ...
                        series: [{
                                ...
                                radius: ['35%', '65%'],
                                center: ['50%', '55%'],
                                data: results.slice(0, 12).map((r, idx) => ({
                    ...
                    option = {
                        ...
                        xAxis: {
                            type: 'category',
                            data: results.slice(0, 30).map((r) => String(r[labelKey] ?? '').slice(0, 20)),
                        ...
                        series: numericKeys.slice(0, 3).map((nk, idx) => ({
                            ...
                            type: 'line',
                            smooth: true,
                            data: results.slice(0, 30).map((r) => r[nk] ?? 0),
                    ...
                    option = {
                        ...
                        xAxis: {
                            type: 'category',
                            data: results.slice(0, 20).map((r) => String(r[labelKey] ?? '').slice(0, 20)),
                        ...
                        series: numericKeys.slice(0, 3).map((nk, idx) => ({
                            name: nk,
                            type: 'bar',
                            data: results.slice(0, 20).map((r) => r[nk] ?? 0),
                            itemStyle: {
