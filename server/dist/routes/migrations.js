...
const SchemaMigratorService_1 = require("../services/SchemaMigratorService");
const router = (0, express_1.Router)();
// Post a dry-run migration query
router.post('/api/:conn/:db/:coll/migrations/dry-run', async function (req, res) {
    ...
    try {
        ...
        const dryRunResult = await SchemaMigratorService_1.schemaMigratorService.dryRunMigration(mongoDb, req.params.coll, prompt, req.query.locale || 'en');
        res.status(200).json({
            success: true,
            result: dryRunResult
        });
...
router.post('/api/:conn/:db/:coll/migrations/execute', async function (req, res) {
    ...
    try {
        ...
        const executionResult = await SchemaMigratorService_1.schemaMigratorService.executeMigration(mongoDb, req.params.coll, pipeline);
        // 2. Synchronize related collections if requested
        const coordinatedResults = [];
        if (coordinatedUpdates && Array.isArray(coordinatedUpdates)) {
            for (const update of coordinatedUpdates) {
                if (update.collectionName && Array.isArray(update.pipeline) && update.pipeline.length > 0) {
                    const res = await SchemaMigratorService_1.schemaMigratorService.executeMigration(mongoDb, update.collectionName, update.pipeline);
...
router.get('/api/:conn/:db/:coll/migrations/suggestions', async function (req, res) {
    const connection_list = MongoService_1.mongoService.getConnections();
    if (!connection_list || !connection_list[req.params.conn]) {
        return res.status(400).json({ msg: 'Invalid connection name' });
    }
    try {
        ...
        const suggestions = await SchemaMigratorService_1.schemaMigratorService.generateSuggestions(mongoDb, req.params.coll, locale);
        res.status(200).json({
            success: true,
            suggestions
        });
