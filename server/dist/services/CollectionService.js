...
class CollectionService {
    ...
    async getSchema(mongoDb, collectionName) {
        ...
        return Object.entries(fieldMap).map(([field, info]) => ({ field, type: info.type, example: info.example }));
    }
    async getAnalysis(mongoDb, collectionName) {
        const sample = await mongoDb.collection(collectionName).aggregate([{ $sample: { size: 100 } }]).toArray();
        const analysis = {};
        sample.forEach((doc) => {
            Object.entries(doc).forEach(([key, value]) => {
        ...
        return Object.entries(analysis).map(([field, data]) => {
    ...
    async performAiAnalysis(mongoDb, dbName, collName, customPrompt) {
