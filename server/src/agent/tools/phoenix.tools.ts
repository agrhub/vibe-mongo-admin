 ...
 */
async function getPhoenixMcpClient(): Promise<Client | null> {
  if (phoenixMcpClient) return phoenixMcpClient;
  ...
  try {
    ...
    const localMcpPath = path.join(process.cwd(), 'node_modules/@arizeai/phoenix-mcp/build/index.js');
    let mcpCommand = 'npx';
    ...
    const client = new Client(
      { name: 'phoenix-agent-client', version: '1.0.0' },
      { capabilities: {} }
    );
 ...
 */
async function callPhoenixTool(toolName: string, args: any): Promise<any> {
  ...
  try {
    ...
    if (response.isError) {
      const msg = content.find((c: any) => c.type === 'text')?.text || 'Unknown Phoenix MCP error';
      throw new Error(msg);
    }
 ...
 */
export async function listPhoenixTools(): Promise<string[]> {
  const client = await getPhoenixMcpClient();
  if (!client) return [];
  try {
    const toolsResult = await client.listTools();
    console.log('[PhoenixMCP] Tool Schemas:', JSON.stringify(toolsResult.tools, null, 2));
    return (toolsResult.tools || []).map((t: any) => t.name);
  } catch (err: any) {
 ...
 */
export async function getSlowQueryTraces(minDurationMs = 500): Promise<any> {
  const client = await getPhoenixMcpClient();
  const projectName = process.env.PHOENIX_PROJECT_NAME || 'vibe-mongo-admin';
  ...
  if (client) {
    ...
    const spanQueryTools = tools.filter(t =>
      (t.includes('span') || t.includes('trace') || t.includes('query')) && 
 ...
 */
export async function getSpanAnnotations(spanId: string): Promise<any> {
  const client = await getPhoenixMcpClient();
  if (!client) {
 ...
 */
export async function collectPhoenixSnapshot(limit = 15): Promise<any> {
  const client = await getPhoenixMcpClient();
  const projectName = process.env.PHOENIX_PROJECT_NAME || 'vibe-mongo-admin';
  const now = new Date();
  ...
  if (client) {
    try {
      ...
      if (!rawSpans) {
        ...
        if (Array.isArray(tracesResult)) rawSpans = tracesResult;
        else if (tracesResult?.traces) rawSpans = tracesResult.traces;
        else if (tracesResult?.data) rawSpans = tracesResult.data;
      }
      ...
      if (rawSpans && rawSpans.length > 0) {
        ...
        const extractLatencyMs = (s: any): number | null => {
          // Direct ms fields
          if (typeof s.durationMs === 'number' && s.durationMs > 0) return s.durationMs;
          if (typeof s.latencyMs === 'number' && s.latencyMs > 0) return s.latencyMs;
          if (typeof s.duration_ms === 'number' && s.duration_ms > 0) return s.duration_ms;
          if (typeof s.latency_ms === 'number' && s.latency_ms > 0) return s.latency_ms;
          // Seconds fields (convert to ms)
          if (typeof s.latencySeconds === 'number' && s.latencySeconds > 0) return s.latencySeconds * 1000;
          if (typeof s.latency_seconds === 'number' && s.latency_seconds > 0) return s.latency_seconds * 1000;
          if (typeof s.duration === 'number' && s.duration > 0) return s.duration > 1000 ? s.duration : s.duration * 1000;
        ...
        const isError = (s: any) =>
          (s.statusCode ?? s.status_code ?? s.status ?? '').toString().toUpperCase() === 'ERROR';
        ...
        const llmSpans  = rawSpans.filter((s: any) => (s.spanKind ?? s.span_kind ?? s.kind ?? '').toString().toLowerCase().includes('llm'));
 ...
 */
export async function getPhoenixFullMetrics(): Promise<any> {
  return collectPhoenixSnapshot();
}

export { callPhoenixTool, getPhoenixMcpClient };
