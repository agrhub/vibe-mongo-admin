...
router.get('/api/:conn/webhooks', async function (req: Request, res: Response) {
  try {
    const config = await webhookStore.getWebhook(req.params.conn);
...
router.post('/api/:conn/webhooks/save', async function (req: Request, res: Response) {
  ...
  try {
    const config = await webhookStore.saveWebhook(
      req.params.conn,
      url,
      slowQueries,
      systemSpikes,
...
router.post('/api/:conn/webhooks/test', async function (req: Request, res: Response) {
  try {
    const report = await webhookService.triggerTestWebhook(
      req.params.conn,
      req.body.url,
      req.body.email,
      req.body.smtpHost,
      req.body.smtpPort ? parseInt(req.body.smtpPort) : undefined,
      req.body.smtpSecure !== undefined ? (req.body.smtpSecure ? 1 : 0) : undefined,
      req.body.smtpUser,
      req.body.smtpPass,
      req.body.smtpSender
    );
...
router.post('/api/:conn/webhooks/delete', async function (req: Request, res: Response) {
  try {
    const deleted = await webhookStore.deleteWebhook(req.params.conn);
