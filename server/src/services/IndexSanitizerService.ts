...
export interface SanitizationResult {
  ...
  indexStatuses: Record<string, 'HEALTHY' | 'REDUNDANT' | 'UNUSED' | 'UNOPTIMIZED'>;
}

export class IndexSanitizerService {
  async sanitizeIndexes(mongoDb: Db, collName: string, activeIndexes: any[]): Promise<SanitizationResult> {
    ...
    const localRedundancies: Record<string, string> = {};
    activeIndexes.forEach((idxA) => {
      if (idxA.name === '_id_') return;
      const keysA = Object.keys(idxA.key);

      activeIndexes.forEach((idxB) => {
        if (idxA.name === idxB.name || idxB.name === '_id_') return;
...
Analyze the above data and return a structured optimization report in the specified JSON format.`;
    ...
    try {
      const result = await geminiService.generateJSON<SanitizationResult>(userPrompt, systemInstruction);
      
      // Ensure result safety and validation defaults
      if (!result.healthScore && result.healthScore !== 0) result.healthScore = 80;
      if (!result.statusDescription) result.statusDescription = 'AI completed the evaluation of active indexes.';
      if (!result.recommendations) result.recommendations = [];
      if (!result.indexStatuses) result.indexStatuses = {};
      ...
      const indexStatuses: Record<string, any> = { '_id_': 'HEALTHY' };
      let healthScore = 100;
...
export const indexSanitizerService = new IndexSanitizerService();
