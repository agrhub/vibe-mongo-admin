...
import { erdMapperService } from './ErdMapperService';
...
export class SchemaMigratorService {
  async dryRunMigration(mongoDb: Db, collectionName: string, prompt: string, locale: string = 'en'): Promise<MigrationDryRunResult> {
    ...
    try {
      const erdResult = await erdMapperService.mapErd(mongoDb, mongoDb.databaseName, collectionName);
...
`;
    ...
    let aiResponse: { pipeline: any[]; explanation: string; impactedRelations?: any[] };
    try {
      aiResponse = await geminiService.generateJSON<{ pipeline: any[]; explanation: string; impactedRelations?: any[] }>(userPrompt, systemPrompt);
    } catch (e: any) {
    ...
    const finalImpactedRelations: any[] = [];
    if (aiResponse.impactedRelations && Array.isArray(aiResponse.impactedRelations)) {
      for (const rel of aiResponse.impactedRelations) {
        try {
          ...
          if (relatedSample) {
            let relatedTransformed: any = relatedSample;
  ...
  async executeMigration(mongoDb: Db, collectionName: string, pipeline: any[]): Promise<{ matchedCount: number; modifiedCount: number }> {
  ...
  async generateSuggestions(mongoDb: Db, collectionName: string, locale: string = 'en'): Promise<string[]> {
    // 1. Fetch a single sample document
    const sample = await mongoDb.collection(collectionName).findOne();
    if (!sample) {
      return [
...
`;
    ...
    try {
      const suggestions = await geminiService.generateJSON<string[]>(userPrompt, systemPrompt);
...
export function sanitizeSampleForAI(doc: any): any {
...
export const schemaMigratorService = new SchemaMigratorService();
