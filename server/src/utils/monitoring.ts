...
const lastResourceAlertTime: { [connName: string]: number } = {};
const lastConnFailureTime: { [connName: string]: number } = {};

// Alert aggregation queue for spam prevention
...
const lastFlushTime: { [connName: string]: number } = {};

async function dispatchOrQueueAlert(connName: string, markdownAlert: string, config: any) {
    if (!config || config.enableGrouping !== 1) {
        // Dispatch immediately
        if (config.url) {
            webhookService.sendNotification(connName, markdownAlert, config.url).catch(err => {
                console.error(`[Alerting-Engine] Immediate Webhook failed:`, err.message);
            });
        }
        if (config.email) {
            webhookService.sendEmailNotification(
                connName,
                markdownAlert,
                config.email,
                config.smtpHost,
...
function checkAndFlushAlerts(connName: string, config: any) {
    if (!config || config.enableGrouping !== 1) return;
...
`).join('\n')}`;
        ...
        if (config.url) {
            webhookService.sendNotification(connName, consolidatedMarkdown, config.url).catch(err => {
            ...
            try {
                ...
                adminDb.serverStatus()
                    .then(function (info: any) {
                        ...
                        webhookStore.getWebhook(key).then((config) => {
                            if (config) {
    ...
    try {
        ...
        if (snapshot) {
            ...
            for (const s of slowSpans) {
                ...
                webhookStore.getWebhook(dbName).then((config) => {
